...
_model_sha1 = {name: (error, checksum, repo_release_tag, caption, paper, ds, img_size, scale, batch, rem) for
    ...
    ('dicenet_w2', '1019', '44e2153f20e75365950e5066701c165a8b928a93', 'v0.0.497', 'DiCENet x2.0', '1906.03516', 'in1k', 224, 0.875, 200, '[sacmehta/EdgeNets]'),  # noqa
...
def load_model(net,
    ...
    """
    import torch
    logging.info("Loading model from {}".format(file_path))
    try:
        if ignore_extra:
            ...
            # skip the output layer, which can have different number of classes
            pretrained_state.pop("output.weight", None)
            pretrained_state.pop("output.bias", None)
            net.load_state_dict(pretrained_state, strict=False)
        else:
            net.load_state_dict(torch.load(file_path))
    except Exception as e:
        logging.info(f"Failed to load state dict from {file_path}. {e}")
