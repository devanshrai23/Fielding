...
log_wrong_prediction = True
log_all_prediction = False
...
def test_pytorch_model(rank, model, test_data, device='cpu', criterion=nn.NLLLoss(), tokenizer=None):
        ...
        for data, target in test_data:
            try:
                if parser.args.task == 'nlp':
                    ...
                    outputs = model(data)
                    # check if this is a tuple for getting model representation
                    ...
                    # topk = (1, 5)
                    topk = (1,)
                    acc = accuracy(output, target, topk=topk, isTest=True)

                    correct += acc[0].item()
                    if len(topk) > 1:
                        top_5 += acc[1].item()
                    if log_wrong_prediction:
                        wrong_predictions += acc[len(topk)]
                    elif log_all_prediction:
                        all_predictions += acc[len(topk)]
    ...
    testRes = {'top_1': correct, 'top_5': top_5,
               'test_loss': sum_loss, 'test_len': test_len, 'wrong_predictions': wrong_predictions,
               'all_predictions': all_predictions}

    return test_loss, acc, acc_5, testRes


def accuracy(output, target, topk=(1,), isTest=False):
    ...
    with torch.no_grad():
        ...
        if log_all_prediction or log_wrong_prediction:
            wrong_predictions = []
            prediction = pred[0]
            target_label = target.reshape(-1)
            if log_all_prediction:
                res.append(list(zip(prediction.cpu().numpy().tolist(), target_label.cpu().numpy().tolist())))
            else:
                for i in range(len(prediction)):
                    if not prediction[i].eq(target_label[i]):
                        wrong_predictions.append((prediction[i].item(), target_label[i].item()))
                res.append(wrong_predictions)
