...
class _training_selector(object):
    ...
    def __init__(self, args, sample_seed=233):
        ...
        self.blacklist = None
        self.initial_reward = OrderedDict()
    ...
    def register_client(self, client_id, feedbacks):
        ...
        if client_id not in self.totalArms:
            ...
            self.initial_reward[client_id] = feedbacks['reward']
    ...
    def pacer(self):
        ...
        self.successfulClients = set()
        # reset self.exploreClients and self.exploitClients
        self.exploreClients = []
        self.exploitClients = []
    ...
    def update_client_util(self, client_id, feedbacks):
        ...
        '''
        if client_id not in self.totalArms:
            self.register_client(client_id, feedbacks)
        # [client_id]['reward']: statistical utility
        # [client_id]['duration']: system utility
        self.totalArms[client_id]['reward'] = feedbacks['reward']
        self.totalArms[client_id]['duration'] = feedbacks['duration']
    ...
    def reset_all_client_util(self):
        for client_id in self.totalArms:
            self.totalArms[client_id]['reward'] = self.initial_reward[client_id]
            self.totalArms[client_id]['time_stamp'] = self.training_round
            self.totalArms[client_id]['count'] = 0
            self.totalArms[client_id]['status'] = True
    ...
    def select_participant(self, num_of_clients, feasible_clients=None):
        ...
        '''
        logging.info(f"Training selector: Selecting {num_of_clients} clients from feasible clients {feasible_clients}")
    ...
    def getTopK(self, numOfSamples, cur_time, feasible_clients):
        ...
        orderedKeys = [x for x in client_list if (int(x) in feasible_clients and (not int(x) in self.blacklist))]
        ...
        if len(sortedClientUtil) == 0:
            self.exploitClients = []
        else:
            if len(sortedClientUtil) > exploitLen:
        ...
        for client_id in self.exploitClients:
            if client_id not in feasible_clients:
                logging.info("WARNING: Training selector: exploitClients {} is not in feasible clients, removed".format(client_id))
        self.exploitClients = [x for x in self.exploitClients if x in feasible_clients]
        pickedClients = []  
        ...
        for client_id in self.exploreClients:
            if client_id not in feasible_clients:
                logging.info("WARNING: Training selector: exploreClients {} is not in feasible clients, removed".format(client_id))
        self.exploreClients = [x for x in self.exploreClients if x in feasible_clients]
        ...
        for i in range(len(pickedClients)):
            client_id = pickedClients[i]
            # logging.info("At round {}, selected client {}, statistical utility {}, system utility {}"
            #     .format(cur_time, client_id, self.totalArms[client_id]['reward'], self.totalArms[client_id]['duration']))
    ...
    def getAllMetricsLength(self):
        return len(self.totalArms)
